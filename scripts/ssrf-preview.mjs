...
function makeResponse(
  url,
  { status = 200, body = webpFixture, onBodyRead } = {},
) {
  const encodedBody =
...
async function probeApprovedUrl(imageUrl) {
  ...
  let result;
  try {
    result = await fetchRemoteImagePreview(
      imageUrl,
      1024 * 1024,
      async (input) => {
        fetchCalled = true;
  ...
  return {
    ...
    preview:
      ...
      result.byteLength === webpFixture.byteLength
        ? "approved image"
        : null,
...
console.log(
  JSON.stringify({
    ...
    unapprovedHost: await probeRejectedUrl(
      "https://cdn.example.com/teddy.webp",
    ),
    ...
    insecureScheme: await probeRejectedUrl(
      "http://storage.googleapis.com/teddy.webp",
    ),
    suffixHost: await probeRejectedUrl(
      "https://storage.googleapis.com.attacker.test/teddy.webp",
    ),
    loopback: await probeRejectedUrl("https://127.0.0.1:3000/health"),
    nonDefaultPort: await probeRejectedUrl(
      "https://storage.googleapis.com:8443/teddy.webp",
    ),
    redirect: {
