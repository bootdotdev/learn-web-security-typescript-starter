...
export function createProductsRouter(deps: Dependencies): Router {
  ...
  router.get("/products/:id", (req, res) => {
    ...
    const quantityInCart = current
      ? (listCartItems(db, current.user.id).find((item) => item.product_id === product.id)
          ?.quantity ?? 0)
      : 0;
    const remainingInventory = Math.max(0, product.inventory_count - quantityInCart);
    res.type("html").send(renderProductPage(current, product, reviews, remainingInventory));
  });
  ...
  router.post("/products/:id/reviews", (req, res) => {
    ...
    if (!Number.isSafeInteger(rating) || rating < 1 || rating > 5 || body === undefined) {
      sendErrorPage(res, 400, "Invalid Review", "Enter a rating and review text.");
      return;
    }
