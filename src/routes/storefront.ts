...
export function createStorefrontRouter(deps: Dependencies): Router {
  ...
  router.get("/", (req, res) => {
    ...
    const cartQuantities = current ? getCartQuantities(current.user.id) : new Map<number, number>();

    res.type("html").send(renderStorefrontPage(current, products, cartQuantities));
  });
  ...
  router.get("/search", (req, res) => {
    ...
    res.type("html").send(renderSearchPage(current, query, products, cartQuantities));
  });

  function getCartQuantities(userId: number): Map<number, number> {
    return new Map(
      listCartItems(db, userId).map((cartItem) => [cartItem.product_id, cartItem.quantity]),
    );
  }
